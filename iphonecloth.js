...
// 			for (i = 9; i < this.num_quads; i++){
// 					this.quads[(i).floor()].setcolor();
// 					}
		}
					
		point = this.points[
			(Math.random()*this.num_y_points-1).floor()+1]
		 	[(Math.random()*this.num_x_points).floor()];
		position = new FastVector(Math.random()*1+.0001, Math.random()*1.2+.01);
		if(position){
			console.log(position);
			point.setCurrent(position);
